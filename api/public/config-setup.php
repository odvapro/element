...
{
	echo json_encode(['success' => false, 'message' => 'Конфигурационный файл уже существует']);
	exit();
}
if (!empty($_POST))
{
	if (!empty($_POST['host']) && !empty($_POST['username']) && !empty($_POST['password']) && !empty($_POST['dbname']) && !empty($_POST['baseUrl']))
	{
		try {
			...
			echo json_encode(['success' => false, 'message' => 'Доступы не верны', 'error' => $e]);
			exit();
		}
	}
}
